                    ...
                    <tr key={r.id} className="text-xs text-slate-700 dark:text-slate-300 hover:bg-slate-50/50 dark:hover:bg-slate-700/20 transition-colors">
                      ...
                      <td className="py-4 px-4">
                        <div className="flex flex-col gap-1.5">
                          {cleanedUp ? (
                            <div className="flex items-center gap-1 text-slate-400" title={`Photo compressed and archived after the retention limit of ${currentTenant?.retentionDays !== undefined ? currentTenant.retentionDays : 30} days (${currentTenant?.planTier || 'Starter'}).`}>
                ...
                {selectedReceipt.photoUrl === "CLEANED_UP" || selectedReceipt.photoUrl === "ARCHIVED" || isOld(selectedReceipt.createdAt) ? (
                  <div className="p-6 text-center max-w-xs m-auto">
                    ...
                    <p className="text-[11px] text-slate-500 dark:text-slate-400 mt-1">
                      In compliance with the retention limit of {currentTenant?.retentionDays !== undefined ? currentTenant.retentionDays : 30} days ({currentTenant?.planTier || 'Starter'} Plan), this photo has been compressed (Gzip) and moved to secure cold-storage to save space.
                    </p>
                  </div>
                ) : (
                  (() => {
