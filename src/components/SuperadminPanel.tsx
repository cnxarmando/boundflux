...
export default function SuperadminPanel() {
  ...
  // Danger Zone & Reset Platform states
    ...
    <div className="space-y-6">
      ...
      {activeSubTab === "tenants" && (
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-3 pt-4 border-t border-slate-200 dark:border-slate-800">
            <button
              type="button"
              onClick={() => setShowDangerZone(!showDangerZone)}
              className="inline-flex items-center gap-2 text-xs font-semibold text-slate-500 hover:text-rose-600 dark:text-slate-400 dark:hover:text-rose-400 transition-colors cursor-pointer px-3 py-1.5 rounded-lg hover:bg-rose-50 dark:hover:bg-rose-950/20"
            >
              <ShieldAlert className="h-4 w-4 shrink-0 text-slate-400" />
              <span>Opções Avançadas de Manutenção do Sistema (Zona de Perigo)</span>
              {showDangerZone ? <ChevronUp className="h-3.5 w-3.5 ml-1" /> : <ChevronDown className="h-3.5 w-3.5 ml-1" />}
            </button>

            {showDangerZone && (
              <div className="mt-3 p-4 bg-rose-50/50 dark:bg-rose-950/20 border border-rose-200 dark:border-rose-900/30 rounded-xl space-y-3 animate-fade-in">
                <div className="flex items-start gap-3">
                  <AlertTriangle className="h-5 w-5 text-rose-600 dark:text-rose-400 shrink-0 mt-0.5" />
                  <div className="space-y-1">
                    <h4 className="text-xs font-bold text-rose-800 dark:text-rose-300">
                      Reset Total da Plataforma (Limpeza de Dados)
                    </h4>
                    <p className="text-[11px] text-rose-700/80 dark:text-rose-400/80 leading-relaxed">
                      Esta opção apaga permanentemente TODAS as empresas cadastradas ({tenants.length}) e todos os dados associados (recibos, BLs, unidades, usuários e logs de auditoria).
                      Use apenas para limpezas de homologação e testes antes de ir para produção.
                    </p>
                  </div>
                </div>
                <div className="pt-1">
                  <button
                    ...
                    className="px-3.5 py-1.5 bg-rose-600 hover:bg-rose-700 text-white rounded-lg text-xs font-bold transition-all shadow-sm flex items-center gap-2 cursor-pointer"
                  >
                    <Trash2 className="h-3.5 w-3.5" />
      ...
      {isResetModalOpen && (
        <div className="fixed inset-0 bg-slate-900/70 backdrop-blur-xs flex items-center justify-center p-4 z-50 animate-fade-in">
          <div className="bg-white dark:bg-slate-850 max-w-md w-full rounded-2xl p-6 border border-slate-200 dark:border-slate-800 shadow-2xl space-y-5">
            <div className="flex items-center gap-3 text-rose-600 dark:text-rose-400">
              ...
              <div>
                <h3 className="text-base font-bold text-slate-900 dark:text-white">
                  Confirmar Reset Total da Plataforma
                </h3>
                <p className="text-xs text-rose-600 dark:text-rose-400 font-semibold">
                  ⚠️ AÇÃO EXTREMA E IRREVERSÍVEL
                </p>
              </div>
            </div>

            <div className="p-3.5 bg-rose-50 dark:bg-rose-950/30 rounded-xl border border-rose-200 dark:border-rose-900/40 text-xs text-rose-800 dark:text-rose-300 space-y-2">
              <p>
                Você está prestes a apagar <strong>todas as {tenants.length} empresa(s)</strong>, usuários, recibos, conhecimentos de embarque, unidades e logs do sistema.
              </p>
            ...
            <div className="space-y-3">
              <label className="flex items-start gap-2.5 cursor-pointer text-xs text-slate-700 dark:text-slate-300 select-none">
                ...
                />
                <span>
                  Estou ciente de que esta ação é irreversível e excluirá permanentemente todos os dados da plataforma.
                </span>
              </label>

              <div>
                <label className="block text-[11px] font-bold text-slate-700 dark:text-slate-300 mb-1">
                  Para confirmar, digite exatamente: <span className="font-mono text-rose-600 dark:text-rose-400">RESETAR TUDO</span>
                </label>
                <input
                  type="text"
                  ...
                  className="w-full text-xs font-mono bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl px-3 py-2.5 text-slate-900 dark:text-white focus:outline-hidden focus:ring-2 focus:ring-rose-500"
                />
              </div>
            </div>
            ...
            <div className="flex justify-end gap-2.5 pt-2 border-t border-slate-100 dark:border-slate-800">
              ...
              <button
                type="button"
                onClick={async () => {
                  if (!resetCheckbox || resetConfirmText !== "RESETAR TUDO") return;
                ...
                disabled={!resetCheckbox || resetConfirmText !== "RESETAR TUDO" || resettingPlatform}
                className="px-4 py-2 bg-rose-600 hover:bg-rose-700 disabled:opacity-50 disabled:cursor-not-allowed text-white rounded-xl text-xs font-bold cursor-pointer transition-all flex items-center gap-2 shadow-sm"
              >
                <Trash2 className="h-3.5 w-3.5" />
                {resettingPlatform ? "Resetando..." : "Confirmar e Resetar Plataforma"}
              </button>
            </div>
          </div>
        </div>
